...
} from '../Chat/index'; // This links the example app to the current SDK implementation
...
export const Chat = ({route}: ChatProps) => {
  ...
  const connect = async () => {
    try {
      ...
      await pusher.init({
        apiKey,
        cluster,
        authEndpoint: `${REACT_APP_BASE_URL_DEV}/dev/message`,
        // ============
  ...
  const onAuthorizer = async (channelName: string, socketId: string) => {
    ...
    const response = await fetch(`${REACT_APP_BASE_URL_DEV}/dev/message`, {
      method: 'POST',
      headers: {
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView>
        <Text style={{color: 'white'}}>{logText}</Text>
      </ScrollView>
    </SafeAreaView>
  );
};
